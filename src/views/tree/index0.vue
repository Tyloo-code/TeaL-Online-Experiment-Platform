    ...
    >
      ...
      <el-table-column label="课程" align="center">
        <template slot-scope="scope">
          ...
          <div>
            <ul>
              <div v-for="(item, index) in scope.row.class.courses" :key="index">
                <el-button @click="toHomeworkStu(item.name, scope.row.id)">{{
                  item.name
                }}</el-button>
              </div>
            </ul>
...
export default {
  ...
  methods: {
    toHomeworkStu(couresName,stuId) {
      localStorage.setItem('_stuCourseName',couresName);
      localStorage.setItem('_stuId',stuId);
      this.$router.push({
        name:'xSheng',
        params:{
          name:localStorage.getItem('_stuCourseName'),
          id:localStorage.getItem('_stuId')
        }
      })
    },
    handleDownload() {
    ...
    fetchData() {
      ...
      getList().then(({ data }) => {
        this.list = data.value.filter((item) => {
          return item.id === this.$store.getters.num;
        });
