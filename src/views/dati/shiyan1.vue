    ...
    <div class="outer">
      ...
      <div class="right">
        ...
        <div v-if="this.myList">
          <div v-for="(item, index) in this.myList.questions" :key="index">
            <div v-if="item.questionType === '图'">
           {{ item.stem }}
              <el-upload
  ...
  methods: {
    ...
    fetchData1() {
      ...
      getQuizzes().then(({ data }) => {
        ...
        console.log(this.myList[0].questions);
        ...
        for (let i = 0; i < this.myList.questions.length; i++) {
          this.questionGroupe[i] = JSON.parse(
            JSON.stringify(this.myList.questions[i])
          );
        }
      });
    },
    ...
    handleSuccess(response) {
      ...
      this.myImagePath = `https://homeworksystem.podolski.cn/${response}`;
    },
    handleError() {
    ...
    submit() {
      ...
      this.myAnswer = JSON.parse(
        JSON.stringify(this.myAnswer).replace(/id/, "quizId")
      );
      this.myAnswer = JSON.parse(
        JSON.stringify(this.myAnswer).replace(/name/g, "student")
      );
      ...
      let quesAnswer = new Array(this.myList.questions.length).fill(
        "请填写答案"
      );
      for (let i = 0; i < this.myAnswer.questions.length; i++) {
        if (this.myAnswer.questions[i].position !== 0) {
          this.myAnswer.questions[i] = JSON.parse(
            JSON.stringify(this.myAnswer.questions[i]).replace(
              /position/g,
              "studentAnswer"
            )
          );
        ...
        if (
          Object.prototype.toString.call(quesAnswer[i]) === "[object Array]"
        ) {
          quesAnswer[i] = JSON.stringify(
            this.myAnswer.questions[i].studentAnswer
          );
        }
      }
