          ...
          <div>
            <ul>
              <div v-for="(item, index) in scope.row.class.courses" :key="index">
                <el-button @click="toHomework1(item.name,scope.row.id)">{{ item.name }}</el-button>
              </div>
            </ul>
          </div>
        </template>
...
export default {
  ...
  methods: {
    toHomework1(courseName,stuId) {
      localStorage.setItem('_toCourseName',courseName)
      localStorage.setItem('_toStuId',stuId)
      this.$router.push({
        name:'Teacherhomework1',
        params:{
          name:localStorage.getItem('_toCourseName'),
          id:localStorage.getItem('_toStuId')
        }
      })
    },
    handleDownload() {
